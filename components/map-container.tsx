...
export function MapContainer({
  ...
  // Coordenadas aproximadas para comunas
  ...
  // Obtener coordenadas para una dirección
  const getCoordinatesForDelivery = (delivery: Delivery): [number, number] => {
    ...
    // Agregar un pequeño offset aleatorio para evitar que los puntos se superpongan exactamente
  ...
  // Actualizar entregas cuando vienen del exterior
  useEffect(() => {
  ...
  const createCustomIcon = (commune: string, isCompleted: boolean) => {
    ...
    return new L.Icon({
      iconUrl: `data:image/svg+xml;base64,${btoa(`
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="${bgColor}" width="32" height="32">
    ...
    <div className={cn("flex flex-col bg-background transition-all duration-300", isFullscreen ? "fixed inset-0 z-50" : "w-full h-full")}>
      <Card className={cn("flex-1 flex flex-col border-0 rounded-lg overflow-hidden", isFullscreen ? "m-0" : "m-4")}>
        ...
        <div className="flex-1 relative bg-slate-100 overflow-hidden">
          {deliveries.length > 0 ? (
            <MapContainer center={[santiagoCenterLat, santiagoCenterLng]} zoom={11} style={{ height: "100%", width: "100%" }} ref={mapRef}>
              <TileLayer
                url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
              />

              {/* Marcadores para cada entrega */}
        ...
        {deliveries.length > 0 && (
          <div className="bg-slate-50 border-t p-4">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
              ...
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="bg-green-50">
                  ✓ {routeStats.completedPoints}
                </Badge>
              </div>
              <div className="flex items-center gap-2">
                <Badge variant="outline" className="bg-gray-50">
                  ⏳ {routeStats.pendingPoints}
                </Badge>
              </div>
            </div>
          </div>
